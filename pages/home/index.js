...
function getSelectedJobs() {
  const array =
    JSON.parse(localStorage.getItem("@webwomen:vagas-selecionadas")) || [];

  return array;
}

const listaFav = getSelectedJobs();

function renderJobsCards(array) {
  array.forEach((job) => {
    const cardVaga = createJobCard(job);
  });
}
...
function renderFavCards(array) {
  const listaVagasSelecionadas = document.querySelector(
    ".lista__vagas_selecionadas"
  );
    ...
    button.addEventListener("click", (event) => {
      ...
      if (vagaExiste) {
        ...
        localStorage.setItem(
          "@webwomen:vagas-selecionadas",
          JSON.stringify(listaFav)
        );

        button.innerText = buttonText[0];

        if (listaFav.length < 1) {
          const textVagas = document.querySelector(
            ".text__vagas--selecionadas"
          );
